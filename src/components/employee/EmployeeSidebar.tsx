...
export function EmployeeSidebar() {
  ...
  const navItems = allNavItems.filter((item) => {
      // If user has pages defined:
      if (user?.pages && user.pages.length > 0) {
        console.log(`[EmployeeSidebar] Checking item: ${item.label}`);
        const hasAccess = user.pages.some(
          (page) => page.toLowerCase().trim() === item.label.toLowerCase().trim()
        );
        console.log(`[EmployeeSidebar] Match for ${item.label}: ${hasAccess}`);
        return hasAccess;
      }
      
      // If user.pages is explicitly empty array -> show nothing
      if (user?.pages && user.pages.length === 0) {
        return false;
      }
